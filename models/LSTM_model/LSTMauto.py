...
def create_modelLSTMauto(df,epochs,batch_size):
...
def riconosci(df, userid, sid, parameter, threshold):
    ...
    filename = f'lstmauto2E_8B/{userid}/{userid}_{sid}_{parameter}_riconosci.csv'
...
def model_and_evaluate_single():
    ...
    print("create model")
    tm.sleep(1)
    lstm, scaler=create_modelLSTMauto(df)
...
def Check(userid,sid,parameter,threshold):
    ...
    results_df.to_csv('precision_recall_results.csv', mode='a', index=False, header=not os.path.exists('precision_recall_results.csv'))
    ...
    filename = f'matriceLSTauto5_8/{username}_{sid}_{parameter}_{threshold}_confusion_matrix.npy'
    np.save(filename, cm)
