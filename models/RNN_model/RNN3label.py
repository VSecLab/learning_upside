...
def create_modelRNNlabel(df,epochs, batch_size):
...
def plotMSEs(df, id, sid, parameter, threshold):
    l = len(df)
    ...
    for user in users:
        ...
        plt.plot(df2['mse'])
        plt.savefig('plots/RNNlabel3_{parameter}_mses_{threshold}_{sid}' + user + '_' + id + '.png')
...
def model_and_evaluate_single():
    ...
    print("create model")
    tm.sleep(1)
    rnn_model, scaler = create_modelRNNlabel(df)
...
def model_and_evaluateRNN3label(userid, sid, features, epochs, batch_size):
    ...
    df = dfs[seqIDs[sid]]
    rnn_model, scaler = create_modelRNNlabel(df,epochs, batch_size)
    ...
    mses.to_csv('resultsMseRNN/RNN3label_{features}_{epochs}_{batch_size}_mse' + username + '_' + str(sid) + '.csv')
...
def check_rnn3label(userid, sid, parameter, threshold, epochs, batch_size):
    ...
    # Creare il modello per uno degli utenti e una delle sequenze
    username = users[userid]
    df = pd.read_csv('resultsMseRNN/RNN3label_{features}_{epochs}_{batch_size}_mse' + username + '_' + str(sid) + '.csv')
    ...
    results_df.to_csv('RNNprecision_recall/RNNlabel3{userid}}_{sid}_{parameter}_{threshold}_{epochs}_{batch_size}_precision_recall_results.csv', mode='a', index=False, header=not os.path.exists('RNNprecision_recall/RNNlabel3{userid}}_{sid}_{parameter}_{threshold}_{epochs}_{batch_size}_precision_recall_results.csv'))
...
# Esempio di utilizzo
#model_and_evaluateRNN3label(0, 19, ['Head_Pitch', 'Head_Yaw', 'Head_Roll'])
# Esegui la verifica con RNN
#check_rnn3label(0, 19,['Head_Pitch', 'Head_Roll', 'Head_Yaw'] ,0.05,2, 8)
